...
module Make (S : Abstract.Storage.STORAGE) = struct
  ...
  let root_for tx label =
    ...
    let data =
      Option.map
        (fun blob ->
          Concepts.Blob.bytes_of_blob blob |> Bytes.to_string |> Concepts.Hash.of_raw_string )
        label
    in
    Ok data
  ...
  class manager storage label head =
    object (self)
        ...
        SI.with_transaction storage (fun tx ->
            match
              Utilities.Atomic.mswap head (fun head ->
                  ...
                  let* reference =
                    Option.map address_of reference
                    |> Utilities.Option.sequence
                    |> Result.map_error (fun e -> Some e)
                  in
                  let* value =
                    Option.map address_of value
                    |> Utilities.Option.sequence
                    |> Result.map_error (fun e -> Some e)
                  in
  ...
  let make storage label =
    ...
    SI.with_transaction storage (fun tx ->
        ...
        match addr with
        | None -> Ok (new manager storage label M.empty |> Protocols.Handle.make)
        | Some addr ->
