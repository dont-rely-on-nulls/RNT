...
functor
  ...
  struct
    ...
    let from_blob blob = Concepts.Codec.Bencode.of_blob blob |> Utilities.Result.fmap of_bencode

    let to_bencode =
      let open Concepts.Codec.Bencode in
      (* Ditto. *)
      let bencode_key k =
        K.encode k |> Concepts.Representation.bytes_of_blob |> String.of_bytes |> fun s -> String s
      in
   ...
   *)
    ...
    let hash_of_keys ks =
      ...
      BatFingerTree.iter
        (fun k -> K.encode k |> Concepts.Representation.bytes_of_blob |> Buffer.add_bytes buffer)
        ks;
...
functor
  ...
  struct
    ...
    let lookup tx k node =
      ...
      match addr with None -> Ok None | Some addr -> retrieve tx addr
  end
