...
type term = Name of string | Abstract of string * term | Apply of term * term
...
type value = Relation of Protocols.Handle.t | Closure of string * term * environment
...
let reduce (Program (term, dir)) =
  ...
  let rec eval env term =
    match term with
    | Name name ->
      begin match BatMap.String.find_opt name env with
      | Some value -> Ok value
      | None ->
          let* handle =
            Directory.find dir name |> fmap (Option.to_result ~none:(Error.unknown_name name))
          in
          Ok (Relation handle)
      end
